...
class Linker:
    ...
    def estimate_parameters_using_expectation_maximisation(
        ...
        extend_block_by_ss_rules: bool = False,
        ...
        Args:
            ...
            comparisons_to_deactivate (list, optional):
                THE BEHAVIOUR OF THIS PARAMETER HAS CHANGED.
                Splink will now only copy parameters back to the model that are unrelated
                to the blocking rule. However, this parameter can be used to control
                which parameters are used in the EM, including parameters related to 
                the blocking rule.
                By default, Splink will not use any parameters related to the blocking rule
                in the EM. To allow Splink to use all parameters in the EM, set this to the 
                empty list ([]).
                
                #By default, splink will
        ...
        em_training_session = EMTrainingSession(
            ...
            semi_supervised_rules = semi_supervised_rules,
            semi_supervised_table = semi_supervised_table,
            extend_block_by_ss_table = extend_block_by_ss_table
    ...
    def register_semi_supervised_table(self, input_data, overwrite=False):
        table_name_physical = "__splink__df_semi_supervised_" + ascii_uid(8)
        splink_dataframe = self.register_table(
            input_data, table_name_physical, overwrite=overwrite
        )
        splink_dataframe.templated_name = "__splink__df_semi_supervised"
        return splink_dataframe
